...
<template>
  <div class="fixed transform -translate-x-1/2 left-1/2 z-50 top-5">
    <nav>
      <div
        class="bg-[#2B2A2A]/10 backdrop-blur-2xl border-[1px] border-paragraph rounded-3xl flex items-center justify-center py-3 px-8 transition-all duration-300 ease-in-out"
      >
        <ul class="flex items-center lg:gap-6 gap-4 text-paragraph">
          <li v-for="(link, index) in links" :key="link.name">
            <a
              ...
              :class="{
                'text-azure': currentHash === link.url,
              }"
              :href="link.url"
